            ...
            {results ? (
              <div className="space-y-4 animate-fade-in">
                ...
                {selectedTradeIndex !== null && results.tradeDetails[selectedTradeIndex] && (
                  <div className="border rounded-lg overflow-hidden animate-slide-up shadow-lg bg-slate-900 border-slate-700">
                    <div className="flex items-center justify-between px-3 sm:px-4 py-2 bg-slate-800 border-b border-slate-700 sticky top-0 z-10">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <BarChart3 className="w-4 h-4 sm:w-5 sm:h-5 text-blue-400" />
                        <div>
                          <h4 className="text-[13px] sm:text-sm font-semibold text-white">
                      ...
                      <div className="flex items-center gap-2 sm:gap-3">
                        <div className="flex items-center gap-1 sm:gap-2">
                          <Label className="text-[11px] sm:text-xs text-gray-400 hidden sm:inline">Tema:</Label>
                          <select
                            value={chartTheme}
                            onChange={(e) => { setChartTheme(e.target.value); savePreferredTheme(e.target.value); }}
                            className="px-2 py-1.5 sm:py-1 text-[11px] sm:text-xs border rounded-lg bg-slate-700 border-slate-600 text-white hover:bg-slate-600 transition-colors min-h-[44px] sm:min-h-0"
                          >
                        ...
                        <button
                          ...
                          className="flex items-center justify-center w-11 h-11 sm:w-8 sm:h-8 text-gray-400 hover:text-white hover:bg-slate-700 rounded-lg transition-colors"
...
function TradeChartWrapper({ trade, config, themeId = "mt5", onClose }: { trade: any; config: any; themeId?: string; onClose?: () => void }) {
  ...
  return (
    <div className="animate-fade-in p-3 sm:p-4 bg-slate-900">
      <SimpleCandleChart
